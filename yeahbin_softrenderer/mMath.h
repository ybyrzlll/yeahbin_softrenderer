...
// 设置摄像机  Look-At方法
static void camera_set_LookAt(Matrix4 *m, const Vector3f *eye, const Vector3f *at, const Vector3f *up) {
...
// 设置摄像机  PHIGS方法
static void camera_set_PHIGS(Matrix4 *m, const Vector3f *eye, const Vector3f *vpn, const Vector3f *up) {
	Vector3f xaxis, yaxis, zaxis;
...
static void matrix_set_perspective(Matrix4 *m, float fovy, float aspect, float zn, float zf) {
...
// 将矢量 x 进行 project 
static void transform_apply(const transform_t &ts, Vector3f &y, const Vector3f &x) {
	y = matrix_apply(ts.MVP, x);
}

// 检查齐次坐标同 cvv 的边界用于视锥裁剪  D3D
static int transform_check_cvv(const Vector3f& v) {
	float w = v.w;
...
//透视除法
static void perspectiveDivide(Vector3f *x) {
...
//视口变换,得到屏幕坐标
static Vector3f transform_viewport(const transform_t *ts, const Vector3f *x) {
	//视口变换
	Vector3f y;
...
//逆透视除法
static void perspectiveDivide_Inverse(Vector3f *x) {
	x->x = x->x * x->w;
	x->y = x->y * x->w;
	x->z = x->z * x->w;
}
...
//逆视口变换 y = x...
static void transform_viewport_Inverse(const transform_t *ts, Vector3f *y, const Vector3f *x) {
	y->x = (2 * x->x / ts->w - 1.0f);
	y->y = (1.0f - 2 * x->y / ts->h);
	y->z = x->z;
}
...
static float  Intersect_Line_Plane(const Vector3f *p1, const Vector3f *p2,
	const int *a) {
...
static float area_of_triangle(Vector3f &a, Vector3f &b, Vector3f &c) {
